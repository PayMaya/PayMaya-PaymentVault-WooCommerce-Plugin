(function(e){e.PayMaya={publicKey:"pk-N6TvoB4GP2kIgNz4OCchCTKYvY5kPQd2HDRSg8rPeQG",sandbox:!1,merchantUrl:{redirect:{success:"",failure:"",cancel:""},payments:{token:"/payment-tokens",create:"/payments",get:""},cards:{},customer:{}},dialogConfig:{width:580,height:500},statusPage:{success:1,failed:0,canceled:2},_server:function(){return 2===this.sandbox?"http://52.76.57.133:8001/v1":1===this.sandbox?"https://pg-sandbox.paymaya.com/payments/v1":"https://api.paymaya.com/payments/v1"},getTokenID:function(a,
b){var c=this.merchantUrl.payments.token,d={},f={};"object"===typeof a?f=a:"string"===typeof a&&(f=this.getCardDetails(a));document.body.data={};d.card=f;this.ajax({url:c,data:d,paymaya:!0,success:function(a){try{document.body.data=JSON.parse(a)}catch(c){throw"Response from PayMaya server is not a valid JSON string.";}b.call(this,e.PayMaya,a)},error:function(a){b.call(this,e.PayMaya,a)}});f=d=null;return this},payments:function(a){if(!this.hasTokenID())throw"TokenID required in order to create a payment.";
var b,c=arguments,d=this.merchantUrl.payments.get,f=this.merchantUrl.payments.create;b=this._checkPaymentArgs(a);"function"===typeof a&&this.ajax({url:d,method:"GET",success:function(b){try{var c=JSON.parse(b);a.call(this,c)}catch(d){throw"AJAX Response is not a valid JSON string.";}}});"object"!==typeof a&&"string"!==typeof a||this.ajax({url:f,data:b,success:function(a){try{var b,d=JSON.parse(a);b=e.PayMaya.extend({id:"",paymentTokenId:"",environment:"development",isPaid:!1,status:"for_detok",amount:0,
currency:"PHP",refunded:!1,captured:!0,amountRefunded:0,createdAt:"",updatedAt:"",description:"",verificationUrl:""},d);2===c.length&&c[1].call(this,b);e.PayMaya._checkResponseStatus(b)}catch(f){console.info(f)}}});b=null;return this},hasTokenID:function(){var a=document.body.data||{};return!this.isEmptyObject(a)&&0<a.paymentTokenId.length?!0:!1},paymentStatusPage:function(){this._getTopHref()!==this._getSelfHref()&&(e.top.location.href=this.decode64(this._getSelfHref()))},onPaymentSuccess:function(){},
onPaymentFailure:function(){},onPaymentCanceled:function(){},ajax:function(a){var b,c,d;this._loadingStatus("show");c=[{name:"Accept",value:"application/json"}];d=this.extend({url:"",method:"POST",async:!0,data:{},paymaya:!1,success:function(){},error:function(){},complete:function(){}},a);!0===d.paymaya?(c.push({name:"Authorization",value:"Basic "+this.base64(this.publicKey+String.fromCharCode(58))}),a=this._server()+d.url):a=d.url;b=this._xhrObj();b.open(d.method,a,d.async);for(var f in c)c.hasOwnProperty(f)&&
b.setRequestHeader(c[f].name,c[f].value);b.responseType="text";b.onreadystatechange=function(){if(4===b.readyState)switch(e.PayMaya._loadingStatus("hide"),b.status){case 200:d.success.call(this,b.responseText,b.responseType,b.statusText);break;default:d.error.call(this,b,b.statusText)}};b.onerror=function(){d.error.call(this,b,"error")};b.ontimeout=function(){d.error.call(this,b,"timeout")};"POST"===d.method?b.send(JSON.stringify(d.data)):b.send(null);return b},extend:function(){var a={},b=!1,c=0,
d=arguments.length;"[object Boolean]"===Object.prototype.toString.call(arguments[0])&&(b=arguments[0],c++);for(var f=function(c){for(var d in c)Object.prototype.hasOwnProperty.call(c,d)&&(b&&"[object Object]"===Object.prototype.toString.call(c[d])?a[d]=this.extend(!0,a[d],c[d]):a[d]=c[d])},c=0;c<d;c++)f(arguments[c]);return a},getCardDetails:function(a){return"string"!==typeof a?{}:this._mapNodeToArray(a,"input[name=number], input[name=expMonth], input[name=expYear], input[name=cvc]")},isEmptyObject:function(a){for(var b in a)if(a.hasOwnProperty(b))return!1;
return!0},createPaymentData:function(a){if(!this.hasTokenID())throw"TokenID required in order to create a payment.";if("string"!==typeof a)return{};a=this._mapNodeToArray(a,"input[name=amount], input[name=currency], input[name=firstName], input[name=middleName], input[name=lastName], input[name=phone], input[name=email], input[name=line1], input[name=line2], input[name=city], input[name=state], input[name=zipCode], input[name=countryCode]");return this._paymentsPostData(document.body.data.paymentTokenId,
a)},showDialog:function(a,b){if("string"!==typeof a&&"function"!==typeof b)return!1;var c=document,d=c.createElement("DIV"),f=c.createElement("IFRAME");d.id="paymaya-container";d.innerHTML='<div id="paymaya-box"><div id="paymaya-border"><div id="paymaya-close"></div></div></div>';d.setAttribute("style","display: block;position: absolute;top: 0px;left: 0px;right: 0px;bottom: 0px;z-index: 9990;background-color: rgba(0,0,0,0.2);");c.body.appendChild(d);c.getElementById("paymaya-border").appendChild(f);
d=c.getElementById("paymaya-box");d.setAttribute("style","display: block;position: relative;margin-right: auto;margin-left: auto;border-radius: 10px 10px 10px 10px;-moz-border-radius: 10px 10px 10px 10px;-webkit-border-radius: 10px 10px 10px 10px;border: 0px solid #000000;z-index: 10000;background-color: rgba(0,0,0,0.3);");c.getElementById("paymaya-border").setAttribute("style","display: block;position: absolute;top: 10px;bottom: 10px;left: 10px;right: 10px;overflow: hidden;background-color: #ffffff;");
c=c.getElementById("paymaya-close");c.setAttribute("style","display: block;position: absolute;top: 0px;right: 0px;left: auto;bottom: auto;width: 24px;height: 24px;z-index: 10010;cursor: pointer;cursor: hand;background-repeat: no-repeat;background-color: transparent;");c.style.backgroundImage="url(data:image/png;base64,)";
c.onclick=function(){e.PayMaya.hideDialog(e.PayMaya.statusPage.canceled)};d.style.width=this.dialogConfig.width+"px";d.style.height=this.dialogConfig.height+"px";d.style.marginTop=this._getMarginTop(d.style.height.toString().replace("px",""));f.src=a;f.width=this._getValueInPixels(d.style.width.toString(),20)+"px";f.height=this._getValueInPixels(d.style.height.toString(),20)+"px";f.frameBorder=0;f.scrolling="no";f.id="paymaya-render";f.setAttribute("style","display: block;position: absolute;top:0px;bottom:0px;left:0px;right:0px;border: none;overflow: hidden;background-color: transparent;");
f.onload=function(a){b.call(this,a)};return!0},hideDialog:function(a){if(!0===isNaN(a))throw"The values for the parameter for hideDialog should come from PayMaya.statusPage object";var b=this._winObj(),c=b.document.getElementById("paymaya-container");null!==c&&b.document.body.removeChild(c);switch(a){case 0:b.PayMaya.onPaymentFailure.apply(b.PayMaya);break;case 1:b.PayMaya.onPaymentSuccess.apply(b.PayMaya);break;case 2:b.PayMaya.onPaymentCanceled.apply(b.PayMaya)}},base64:function(a){var b,c,d,f,
e=0,g=0,h=[];if(!a)return a;a=decodeURIComponent(encodeURIComponent(a));do b=a.charCodeAt(e++),c=a.charCodeAt(e++),d=a.charCodeAt(e++),f=b<<16|c<<8|d,b=f>>18&63,c=f>>12&63,d=f>>6&63,f&=63,h[g++]="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=".charAt(b)+"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=".charAt(c)+"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=".charAt(d)+"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=".charAt(f);
while(e<a.length);e=h.join("");a=a.length%3;return(a?e.slice(0,a-3):e)+"===".slice(a||3)},decode64:function(a){if("string"!==typeof a)return"";if("undefined"!==typeof win){if("undefined"!==typeof win.atob)return decodeURIComponent(decodeURIComponent(win.atob(a)))}else return(new Buffer(a,"base64")).toString("utf-8");var b,c,d,e,l,g=0,h=0,k=[];if(!a)return a;a+="";do b="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=".indexOf(a.charAt(g++)),c="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=".indexOf(a.charAt(g++)),
e="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=".indexOf(a.charAt(g++)),l="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=".indexOf(a.charAt(g++)),d=b<<18|c<<12|e<<6|l,b=d>>16&255,c=d>>8&255,d&=255,64===e?k[h++]=String.fromCharCode(b):64===l?k[h++]=String.fromCharCode(b,c):k[h++]=String.fromCharCode(b,c,d);while(g<a.length);a=k.join("");return decodeURIComponent(encodeURIComponent(a.replace(/\0+$/,"")))},_xhrObj:function(){var a,b=["MSXML2.XMLHTTP.6.0","MSXML2.XMLHTTP.3.0",
"MSXML2.XMLHTTP","Microsoft.XMLHTTP"];if(e.XMLHttpRequest)a=new e.XMLHttpRequest;else if(e.ActiveXObject)for(var c=0;c<b.length;c++){try{a=new e.ActiveXObject(b[c])}catch(d){continue}break}return a},_winObj:function(){return e.top!==e.self?e.top:e},_getTopHref:function(){return this.base64(e.top.location.href)},_getSelfHref:function(){return this.base64(e.self.location.href)},_paymentsPostData:function(a,b){return{paymentTokenId:a,totalAmount:{amount:this._checkObjPropExist(b,"amount",0),currency:this._checkObjPropExist(b,
"currency","")},buyer:{firstName:this._checkObjPropExist(b,"firstName",""),middleName:this._checkObjPropExist(b,"middleName",""),lastName:this._checkObjPropExist(b,"lastName",""),contact:{phone:this._checkObjPropExist(b,"phone",""),email:this._checkObjPropExist(b,"email","")},billingAddress:{line1:this._checkObjPropExist(b,"line1",""),line2:this._checkObjPropExist(b,"line2",""),city:this._checkObjPropExist(b,"city",""),state:this._checkObjPropExist(b,"state",""),zipCode:this._checkObjPropExist(b,
"zipCode",""),countryCode:this._checkObjPropExist(b,"countryCode","")}},redirectUrl:this._getRedirectURL()}},_mapNodeToArray:function(a,b){for(var c={},d=document.querySelector(a).querySelectorAll(b),e=0;e<d.length;e++)c[d[e].name]=d[e].value;return c},_checkObjPropExist:function(a,b,c){return a[b]||c},_getRedirectURL:function(){var a=e.location.protocol+String.fromCharCode(47)+String.fromCharCode(47)+e.location.host;return{success:a+this.merchantUrl.redirect.success,failure:a+this.merchantUrl.redirect.failure,
cancel:a+this.merchantUrl.redirect.cancel}},_checkPaymentArgs:function(a){var b;"object"===typeof a?(b=a||{},b.redirectUrl=this._getRedirectURL()):"string"===typeof a&&(b=this.createPaymentData(a));return b},_getValueInPixels:function(a,b){return parseInt(a.replace("px",""),10)-b},_loadingStatusInsert:function(){var a=e.document.createElement("DIV"),b=e.document.createElement("DIV");a.id="paymaya-ajaxloading";a.setAttribute("style","display: block;position: absolute;top: 0;left: 0;right: 0;bottom: 0;z-index: 9999;background-color: rgba(0,0,0,0.2);");
b.id="paymaya-gif";b.setAttribute("style","display: block;position: relative;margin-left: auto;margin-right: auto;width: 100px;height: 100px;background-repeat: no-repeat;background-color: transparent;");b.style.backgroundImage="url(data:image/gif;base64,)";
b.style.marginTop=this._getMarginTop(b.style.height.replace("px",""));a.appendChild(b);e.document.body.appendChild(a)},_loadingStatus:function(a){"show"===a?this._loadingStatusInsert():e.document.body.removeChild(e.document.querySelector("#paymaya-ajaxloading"))},_getMarginTop:function(a){var b=e.outerHeight-parseInt(a,10);return b>=parseInt(a,10)?b/4+"px":"0px"},_checkResponseStatus:function(a){if(7<a.verificationUrl.length)return e.PayMaya.showDialog(a.verificationUrl,function(){}),!0;"PAYMENT_SUCCESS"===
a.status&&!0===a.isPaid?e.PayMaya.hideDialog(e.PayMaya.statusPage.success):e.PayMaya.hideDialog(e.PayMaya.statusPage.failed);return!1}}})(window);
