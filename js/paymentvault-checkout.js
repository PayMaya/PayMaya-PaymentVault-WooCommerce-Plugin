	...
	var wc_checkout_form = {
		...
		submit: function() {
			...
			if ( $form.triggerHandler( 'checkout_place_order' ) !== false && $form.triggerHandler( 'checkout_place_order_' + wc_checkout_form.get_payment_method() ) !== false ) {
				...
				var gatewayName = $('#wc-gateway-name').val(),
						gatewayNonce = $('#wc-gateway-nonce').val().replace(/\s/g,""),
						gatewayID = $('#wc-gateway-id'),
						gatewaySandbox = $('#wc-gateway-sandbox'),
						expMonth = $form.find('#'+gatewayName+'-card-expiry').val().replace(/\s/g,"").split(String.fromCharCode(47));
				
				PayMaya.publicKey = gatewayNonce;
				PayMaya.sandbox = (gatewaySandbox.prop('value') === 'yes')? 1 : 0;
				
				var card = {
					number: $form.find('#'+gatewayName+'-card-number').val().replace(/\s/g,""),
					expMonth: (expMonth.length === 2)? expMonth[0] : " ",
					expYear: (expMonth.length === 2)? expMonth[1] : " ",
					cvc: $form.find('#'+gatewayName+'-card-cvc').val().replace(/\s/g,"")
				};
